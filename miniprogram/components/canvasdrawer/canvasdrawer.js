Component({
  ...
  methods: {
    ...
    drawImage(params) {
      ...
      var realWidth = url.width;
      var realHeight = url.height;
      ...
      if (realWidth <= realHeight) {
        sWidth = realWidth;
        sHeight = realWidth / scale;
        sy = (realHeight - sHeight) / 2;

      } else {
        sHeight = realHeight;
        sWidth = realHeight * scale
        sx = (realWidth - sWidth) / 2
      }
      ...
      if (borderRadius) {
        ...
        this.ctx.beginPath()
        this.ctx.arc(left + borderRadius, top + borderRadius, borderRadius, 0, 2 * Math.PI, false)
        this.ctx.clip()
        this.ctx.drawImage(url.path,sx, sy, sWidth, sHeight, left, top, width, height);
        ...
        if (deg !== 0) {
          ...
          this.ctx.drawImage(url.path, -width / 2, -height / 2, width, height)
        } else {
